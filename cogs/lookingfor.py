...
class lookingfor(commands.Cog):
    ...
    async def review(self, ctx, category: str):
        ...
        if category == "scrimna":
            message = await ctx.send("Working to fetch the list! This may take a few minutes.")
            pages = Configuration.paginate(", ".join(Configuration.getConfigVar(ctx.guild.id, "SCRIMNA")))
            embed = discord.Embed(title=f"This is {category}'s list of words to keep an eye out", description=f"```{pages}```", color=0xff7171)
            await ctx.send(embed=embed)
            await asyncio.sleep(5)
            await message.delete()
        if category == "scrimeu":
            message = await ctx.send("Working to fetch the list! This may take a few minutes.")
            pages = Configuration.paginate(", ".join(Configuration.getConfigVar(ctx.guild.id, "SCRIMEU")))
            embed = discord.Embed(title=f"This is {category}'s list of words to keep an eye out", description=f"```{pages}```", color=0xff7171)
            await ctx.send(embed=embed)
            await asyncio.sleep(5)
            await message.delete()
        if category == "scrimother":
            message = await ctx.send("Working to fetch the list! This may take a few minutes.")
            pages = Configuration.paginate(", ".join(Configuration.getConfigVar(ctx.guild.id, "SCRIMOTHER")))
            embed = discord.Embed(title=f"This is {category}'s list of words to keep an eye out", description=f"```{pages}```", color=0xff7171)
            await ctx.send(embed=embed)
            await asyncio.sleep(5)
            await message.delete()
